...
const catStartingFromIndexFiveExists = animalStrings.some((animal) => {
  animal[4];
});
...
const integerArray = animalStrings.map((animal) => animal.length);
const sumOfAllAnimalCharacters = integerArray.reduce((acc, currentAnimal) => {
  return acc + currentAnimal;
});
